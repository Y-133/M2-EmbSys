...
float tempCe; //final temperature in degree celsius after calibration
...
void loop() 
{
  ...
  if (go == 0) 
  {
    ...
    if (digitalRead(1) == 0 && poz >= 2) 
    {
      poz = poz - 2;
    }
    
    if (digitalRead(2) == 0 && poz <= 46) 
    {
      poz = poz + 2;
    }
    //Bullet fire
    if (digitalRead(4) == 0 && postoji == 0)
    {
      postoji = 1;
      metx = 6;
      mety = poz + 8;
      tone(13, 1200, 20); //<bullet sound
    }
    if (postoji == 1)
    {
      metx = metx + 8 ;
...
void takeReading()
{
  ...
  tempCe=temp1-calibration; // Final temperature in celsius
  if(tempCe > 125 )
    motor_on(255);
  else if( 110 < tempCe < 125)
    motor_on(128);
  else if( 101 < tempCe < 110)
    motor_on(64);
}
//-----------------------------------------------------------------------------------------------
// This function turns on the motors
...
void speedon(int speed) 
{
  ...
  Serial.println("Motor is ON, speed is");
  Serial.print(speed);
}

// This function lets you control speed of the motors
void speedoff(int speed) 
{  
  ...
  Serial.println("Motor is OFF, speed is");
  Serial.print(speed);
}
//------------------------------------------------------------------------------------
